...
import { TEST_PORTFOLIOS, TEST_SECURITIES, type Portfolio, type Security } from "@/services/testData";
    ...
    if (useTestData) {
      ...
      if (response.success) {
        ...
        toast({
          ...
          description: response.error || `Portfolio ${selectedPortfolio} could not be found.`,
          variant: "destructive",
        });
      }
    }
    ...
    if (useTestData) {
      ...
      if (response.success) {
        ...
        toast({
          ...
          description: response.error || `Security ${selectedSecurity} could not be found.`,
          variant: "destructive",
        });
      }
    }
          ...
          <TabsContent value="portfolios" className="space-y-4">
                    ...
                    .map((portfolio) => (
                      <SelectItem key={portfolio.code} value={portfolio.code}>
                        <div className="flex flex-col">
                          <span className="font-medium">{portfolio.code} - {portfolio.name}</span>
            ...
            <div className="space-y-2">
                  ...
                  <div key={code} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium">{details?.name || code}</span>
                      </div>
                      {details && (
              ...
              {portfolioCodes.length === 0 && (
                <p className="text-muted-foreground text-sm text-center py-4">No portfolios selected</p>
              )}
            </div>
          </TabsContent>
          ...
          <TabsContent value="securities" className="space-y-4">
                  ...
                  {availableSecurities
                    ...
                    .map((security) => (
                      <SelectItem key={security.code} value={security.code}>
                        <div className="flex flex-col">
                          <span className="font-medium">{security.code} - {security.name}</span>
                          <span className="text-xs text-muted-foreground">{security.sector} • {security.exchange}</span>
                        </div>
                      </SelectItem>
                    ))}
                </SelectContent>
            ...
            <div className="space-y-2">
                ...
                return (
                  <div key={code} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                    <div className="flex-1">
                      ...
                      {details && (
                        <p className="text-xs text-muted-foreground mt-1">{details.sector} • {details.exchange}</p>
                      )}
                    </div>
                    <Button
                      variant="ghost"
              ...
              {securityCodes.length === 0 && (
                <p className="text-muted-foreground text-sm text-center py-4">No securities selected</p>
              )}
            </div>
          </TabsContent>
        </Tabs>
